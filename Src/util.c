...
extern uint32_t buzzerTimer;// vam added for delay_periodic_loop
...
void poweroff(void) {
  ...
  saveConfig();
	 //VAM  MAYBE TURN OFF BUZZER WHEN DISCONNECT
	beepCount(0,0,0); 
...
void poweroffPressCheck(void) {
    ...
    if(HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN)) {
      ...
      if (cnt_press >= 5 * 100   ) {                         // Check if press is more than 5 sec
        HAL_Delay(1000);
        if (HAL_GPIO_ReadPin(BUTTON_PORT, BUTTON_PIN) && MAX_CURRENT_SPEED_ADJUST_ON) {  // Double press: Adjust Max Current, Max Speed
...
//	uint32_t loop_ticks = loop_ms*1000*ticks;
...
//		DWT->CTRL |=   DWT_CTRL_CYCCNTENA_Msk;
//		
//		printf("initial_ticks %i loop_ticks %i ticks %i ",initial_ticks, loop_ticks,ticks);
//	  bflag = false;
//		DWT->CYCCNT = 0;
//		//HAL_Delay(1);
//		//printf("CYCCNT after 1 ms %i ", DWT->CYCCNT);
//	}
//	//initial_ticks = initial_ticks + loop_ticks;
//		while ((DWT->CYCCNT - initial_ticks ) < loop_ticks); 
//	  initial_ticks = DWT->CYCCNT ;
//	//printf("initial_ticks %i CYCCNT %i ",initial_ticks, DWT->CYCCNT);
//	
//	
//}
_Bool update_periodic_loop( volatile uint32_t loop_ms)
{
  //This routine creates a ms delay for a periodic loop.   It uses a micro second debug clock in a while loop to wait until the total
	//loop time equals loop_ms.  It differs from a straight HAL_Delay which simply adds a fixed delay to other loop delay.
	//This routine allows the delay to be smaller to compensate for larger loop delays caused by printf etc.
  ...
  static _Bool bflag = true; //initial pass true
	if(bflag)
	{
		loop_ticks = loop_ms*16;
		last_buzzerTimer = buzzerTimer;
		bflag = false;
	}
	// Return true if at least loop_ms have elapsed since last loop was run.
		if((buzzerTimer-last_buzzerTimer )< loop_ticks){return false; }//{printf("%i ",buzzerTimer-initial_ticks);} 
	  else
		{ 
			last_buzzerTimer = buzzerTimer ;
			return true;
